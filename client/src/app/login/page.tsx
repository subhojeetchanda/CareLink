...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Fetch profile from backend to get the role
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#030712] text-slate-900 dark:text-slate-50 flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans transition-colors duration-300">
      ...
      <div className="w-full max-w-md space-y-8 rounded-3xl bg-white dark:bg-slate-900/50 p-8 shadow-xl dark:shadow-2xl backdrop-blur-xl border border-slate-200 dark:border-white/10 relative z-10 transition-colors">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
            ...
            <Link href="/register" className="font-medium text-cyan-600 dark:text-cyan-400 hover:text-cyan-500 dark:hover:text-cyan-300 transition-colors">
              Sign up today
            </Link>
          </p>
        </div>
